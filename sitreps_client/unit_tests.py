...
def _get_testing_tools(ci_log: str):
    ...
    return set(tools_found)


def _get_tests_count(ci_log: str, testing_tools: Optional[str] = None) -> int:  # noqa: C901
    ...
    if testing_tools:
        tools = testing_tools.split(",")
    else:
        tools = _get_testing_tools(ci_log)
        LOGGER.info(f"[Travis] Testing tools: {tools}")

        if len(tools) > 1:
            LOGGER.warning(
                f"[Travis]: Multiple testing tools detected ({tools}),"
                f"You can set specific in setting like 'testing_tools: foo'."
                f"Known testing tools are {KNOWN_TESTING_TOOLS}."
            )
        if len(tools) == 0:
            LOGGER.warning("No testing tool detected.")
    ...
    for line in escape_ansi(ci_log).splitlines():
        ...
        if "maven" in tools and all(
            ...
            try:
                tests_count += int(re.search("Tests run: ([0-9]+),", line).group(1))  # type: ignore
            except (IndexError, AttributeError):
                pass
            continue
...
class GHActionUnitTests:
    ...
    def __init__(
        self, repo_slug: str, github_token: str, branch: str = "master", workflow: str = None
    ):
    ...
    def get_runs(self):
        ...
        if not resp.ok:
            LOGGER.error(f"GHActionUnitTests: Unable to fetch runs: {resp.reason}")
        ...
        if not runs:
            LOGGER.info(
                f"GHActionUnitTests: runs not found for '{self.repo_slug}' against"
        ...
        if self.workflow:
            ...
            if not runs:
                LOGGER.info(
                    f"GHActionUnitTests: runs not found with workflow name {self.workflow}."
                )
        else:
    ...
    def get_logs(self):
        ...
        if not resp.ok:
            LOGGER.error(f"GHActionUnitTests: Unable to fetch logs {resp.reason}")
            raise SitrepsError(f"Unable to fetch logs: {resp.reason}")

        zipfile_name = f"/tmp/github_action_log_{uuid4()}.zip"
    ...
    def get_num_of_tests(self, testing_tools: Optional[str] = None) -> Optional[int]:
        """Return number of unit tests."""
        logs = self.get_logs()

        if not logs:
            LOGGER.warning("GHActionUnitTests: No logs/runs collected.")
            return 0
        return sum(
            _get_tests_count(log.decode(), testing_tools=testing_tools) for log in self.get_logs()
        )
...
class TravisUnitTests:
    ...
    def get_logs(self) -> Generator[str, None, None]:
        try:
            ...
            LOGGER.warning(msg)
            raise SitrepsError(f"[Travis]: {msg}, Error:{e}")

        builds = repo.get_builds(params={"branch": self.branch, "state": "passed", "limit": 2})
        # Above query check for best match not exact
        builds = [
            build
            for build in builds.builds
            if (build.branch.name == self.branch and build.state == "passed")
        ]
        if not builds:
            raise SitrepsError(
    ...
    def get_num_of_tests(self, testing_tools: Optional[str] = None) -> Optional[int]:
        ...
        # pylint: disable=broad-except
            ...
            msg = f"[TravisCI-{self.repo_slug}]: {exc}"
            LOGGER.error(msg)
            return None

        for ci_log in log_generator:
            num_of_tests = _get_tests_count(ci_log, testing_tools=testing_tools)
            if num_of_tests > 0:
                return num_of_tests
        return 0
...
class CIUnitTests:
    ...
    def get_num_of_tests(self, testing_tools: Optional[str] = None) -> Optional[int]:
        ...
        # pylint: disable=broad-except
            ...
            return None
        num_of_tests = _get_tests_count(string, testing_tools=testing_tools)
        return num_of_tests
...
def get_unit_tests(travis=None, gh_action=None, jenkins=None):
    ...
    if jenkins:
        ...
        unit_tests["jenkins"] = CIUnitTests(
            url=jenkins.get("url"), ci_downloader=jenkins_downloader
        )
